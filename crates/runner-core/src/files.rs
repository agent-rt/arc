//! File transfer — OS-agnostic (`tokio::fs`), shared by every runner. Byte-offset
//! read/write gives chunked, resumable transfer of files larger than one
//! protocol frame; the controller drives chunking by looping over offsets.

use arc_proto::wire::{FileHash, Reply};
use blake2::{Blake2s256, Digest};
use tokio::io::{AsyncReadExt, AsyncSeekExt, AsyncWriteExt};

use crate::{RemoteResult, not_found, os_error};
...
/// Lists file paths (relative to `root`, forward-slash) recursively. Skips
/// [`SKIP_DIRS`] unless `all` (then only `.git`). Missing root → empty listing.
...
/// hashes to `None` (so the controller knows to transfer it).
    ...
    let read_result = if max_len == 0 {
        ...
        loop {
            match file.read(&mut buffer[filled..]).await {
                ...
                Err(e) => return Err(os_error(format!("read {path}: {e}"))),
            }
        }
...
/// `offset > 0` opens it and seeks there. Parent directories are created.
